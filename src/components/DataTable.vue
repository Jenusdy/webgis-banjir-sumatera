        ...
        <tbody>
          <tr v-for="(item, index) in paginatedData" :key="index" class="hover:bg-slate-50 transition-colors">
            ...
            <td class="px-2 py-1 sm:px-3 sm:py-1 border-b border-slate-200 text-center min-w-[110px]">
              <div class="flex flex-col gap-0.5 text-[9px] sm:text-[10px]">
                <div class="text-red-600 font-semibold">T: {{ item.properties.infra_pendidikan_negeri_terdampak }}</div>
                <div class="text-green-600">TT: {{ item.properties.infra_pendidikan_negeri_tidak }}</div>
              </div>
            </td>
            <td class="px-2 py-1 sm:px-3 sm:py-1 border-b border-slate-200 text-center min-w-[110px]">
              <div class="flex flex-col gap-0.5 text-[9px] sm:text-[10px]">
                <div class="text-red-600 font-semibold">T: {{ item.properties.infra_pendidikan_swasta_terdampak }}</div>
            ...
            <td class="px-2 py-2 sm:px-3 sm:py-2 border-b border-slate-200 text-center min-w-[110px]">
              <div class="text-[10px] sm:text-xs text-green-700">{{ item.properties.area_lbs_tidak_terdampak.toFixed(2)
              }}</div>
            </td>
            <td class="px-2 py-2 sm:px-3 sm:py-2 border-b border-slate-200 text-center min-w-[100px]">
              <div class="text-[10px] sm:text-xs text-red-700">{{ item.properties.area_lbs_terdampak.toFixed(2) }}</div>
            </td>
          </tr>
        </tbody>
      </table>
...
import dataStatistik from '../data/data-statistik.json'
...
const uniqueProvinsi = computed(() => {
  const provinsi = dataStatistik.features.map(f => f.properties.namaprov)
  return [...new Set(provinsi)].sort()
})

const filteredData = computed(() => {
  let filtered = dataStatistik.features
